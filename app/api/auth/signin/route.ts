...
export async function POST(req: Request){
  try {
    ...
    const d = db();
    const user = await d.prepare('SELECT id,email,password,name FROM users WHERE email=?').bind(body.email).first<{id:string;email:string;password:string;name:string}>();
